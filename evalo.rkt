...
(define (evalo/propagation exp env val store store~ next-address next-address~ cont)
  (fresh (intermediate-val label bval store^ next-address^)
         (evalo-env exp env intermediate-val store store^ next-address next-address^)
         (conde ((== (jbrk ,label ,bval) intermediate-val)
                 (== `(,val ,store~ ,next-address~) `(,intermidiate-val ,store^ ,next-address^)))
                ((=/= (jbrk ,label ,bval) intermediate-val)
                 (cont)))))
...
(define (evalo-env-list exp-list env value-list store store~ next-address next-address~)
  (conde ((== exp-list `()) (== value-list `()) (== store store~) (== next-address next-address~))
         ((fresh (exp exp-rest val value-rest store^ next-address^)
                 ...
                 (conde ((fresh (label b-value)
                                (== val (jbrk label b-value))
                                (== `(,next-address~ ,store~) `(,next-address^ ,store^))))
                        )
